...
public class MocapDataClient
{
	...
	public MocapDataClient(int networkInterfaceToUse)
	{
		try
		{
			...
			System.out
			        .println("MOCAP: Network adapters found:\n-----------------------------------");

			while (enumeration.hasMoreElements())
			{
				NetworkInterface networkInterface = enumeration.nextElement();

				System.out.println("MOCAP: - Index: " + networkInterface.getIndex()
			...
			if (NETWORK_IF_TO_USE >= 0)
			{
				...
				System.out.println(
				        "MOCAP: Network Interface Name: " + networkInterface.getDisplayName());
			}
	...
	public void sendFrameRequest() throws IOException
	{
		...
		DatagramPacket packet = new DatagramPacket(buffer.array(), buffer.array().length,
		        InetAddress.getByName(mocapIP), commandPort);
		...
		public void run()
		{
			...
			while (running.get())
			{
				try
				{
					...
					ArrayList<MocapMarker> listOfLabeledMocapMarkers = recvPacket
					        .getLabeledMarkers();
					ArrayList<MocapMarker> listOfUnlabeledMocapMarker = recvPacket
					        .getUnLabeledMarkers();

					if (firstTime)
					{
						System.out.println(
						        "MOCAP: Data was received from MOCAP Server! # Of Rigid bodies detected: "
						                + lisftOfRigidbodies.size());
						firstTime = false;
					...
					updateListeners(lisftOfRigidbodies,listOfMarkerSets,listOfLabeledMocapMarkers,
					        listOfUnlabeledMocapMarker);
				}
				catch (IOException e)
				{
	...
	protected void updateRigidBodiesListeners(ArrayList<MocapRigidBody> listOfRigidbodies)
	{
		...
		ArrayList<MocapRigidbodiesListener> listeners = (ArrayList<MocapRigidbodiesListener>) listOfMocapRigidBodiesListeners
		        .clone();
	...
	{
		ArrayList<MocapMarkerSetListener> listeners = (ArrayList<MocapMarkerSetListener>) listOfMocapMarkerSetListeners
		        .clone();
	...
	{
		ArrayList<MocapLabeledMarkersListener> listeners = (ArrayList<MocapLabeledMarkersListener>) listOfMocapLabeledMarkersListeners
		        .clone();
	...
	protected void updateUnLabeledMocapMarkersListeners(
	        ArrayList<MocapMarker> listOfUnLabeledMarkers)
	{
		ArrayList<MocapUnLabeledMarkersListener> listeners = (ArrayList<MocapUnLabeledMarkersListener>) listOfMocapUnLabeledMarkersListeners
		        .clone();
